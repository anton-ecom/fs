...
exports.runGCSDemo = runGCSDemo;
const gcs_1 = require("../dist/promises/gcs");
const fs = require("fs");
...
async function runGCSDemo() {
    ...
    try {
        ...
        const gcsFS = (0, gcs_1.createGCSFileSystem)(gcsOptions);
...
- Capability composition over inheritance`;
        ...
        await Promise.all(concurrentFiles.map(file => gcsFS.writeFile(file.name, file.content)));
        console.timeEnd('Concurrent upload');
        console.time('Concurrent read');
        const readResults = await Promise.all(concurrentFiles.map(file => gcsFS.readFile(file.name)));
