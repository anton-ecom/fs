...
/**
 * S3-based filesystem implementation
 * 
...
export class S3FileSystem implements IFileSystem {
  ...
  /**
   * Synchronously check if a file exists in S3
  ...
  /**
   * Synchronously read a file from S3
   * @param path File path in the bucket
   */
  readFileSync(path: string): string {
    try {
      ...
      let content: string;
      if (response.Body) {
        content = this.makeSync(() => this.bodyToString(response.Body));
      } else {
        throw new Error(`No content returned for ${path}`);
      }
  ...
  /**
   * Synchronously write a file to S3
   * @param path File path in the bucket
  ...
  /**
   * Synchronously delete a file from S3
   * @param path File path in the bucket
   */
  ...
  /**
   * Synchronously ensure a directory exists (no-op for S3)
  ...
  private makeSync<T>(asyncFn: () => Promise<T>): T {
    ...
    if (result === undefined) {
      throw new Error('Unexpected undefined result from S3 operation');
    }
...
/**
 * Create a new S3 filesystem instance
